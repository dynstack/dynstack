...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let problem = match args
        .next()
        .expect("Expected problem type (HS, RM, CS)")
        .as_str()
    {
        "HS" => Problem::Hotstorage,
        "RM" => Problem::RollingMill,
        "CS" => Problem::CraneScheduling,
        _ => panic!("Expected problem type (HS, RM, CS)"),
    };
        ...
        let response = match problem {
            Problem::Hotstorage => {
                if let Some(new_schedule) =
                    hotstorage::plan_moves(&protobuf::parse_from_bytes(&msg[2])?, opt_type)
                {
            ...
            Problem::RollingMill => {
                if let Some(new_plan) =
                    rollingmill::plan_moves(&protobuf::parse_from_bytes(&msg[2])?)
                {
            ...
            Problem::CraneScheduling => {
                if let Some(new_soluiton) =
                    cranescheduling::plan_moves(&protobuf::parse_from_bytes(&msg[2])?)
                {
                    Some(new_soluiton.write_to_bytes()?)
                } else {
                    None
                }
            }
